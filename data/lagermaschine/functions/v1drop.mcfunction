...
tellraw @p[distance=..2,scores={PZLagerM.1Ausl=1}] ["Lagermaschine.1: ",{"text":"\nausgelagert == ","bold":true},{"entity":"@s","nbt":"Items[{Slot:15b}].id","color":"dark_purple","bold":true}]
...
summon minecraft:item ~ ~ ~ {PickupDelay:32767s,Item:{id:"minecraft:stone",Count:1b},Tags:["EtiLagerM.1Drop"]}
...
execute if score VarLagerM.1Stapel PZLagerM.1Stapel matches 1.. store result storage lagermaschine:v1daten "EigLagerM.1Lager"[0].Count int 1 run scoreboard players remove VarLagerM.1Stapel PZLagerM.1Stapel 1
