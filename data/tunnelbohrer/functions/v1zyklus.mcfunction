...
execute as @e[type=minecraft:armor_stand,tag=EtiTuBohr.1Bohrer] at @s if entity @e[distance=..3,type=minecraft:item,tag=EtiTuBohr.1Kohle,sort=nearest,limit=1] store result score @s PZTuBohr.1Bohr run scoreboard players operation @e[distance=..3,type=minecraft:item,tag=EtiTuBohr.1Kohle,sort=nearest,limit=1] PZTuBohr.1Bohr *= KonstTuBohr.1TAUSEND PZTuBohr.1Bohr
