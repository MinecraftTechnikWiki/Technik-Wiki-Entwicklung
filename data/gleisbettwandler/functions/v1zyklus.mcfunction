...
execute at @e[type=minecraft:area_effect_cloud,tag=EtiGlBW.1Alle] run setblock ~ ~-1.5 ~-1 minecraft:air replace
execute at @e[type=minecraft:area_effect_cloud,tag=EtiGlBW.1Alle] run setblock ~ ~-1.5 ~-1 minecraft:redstone_block replace
...
execute at @e[type=minecraft:area_effect_cloud,tag=EtiGlBW.1Alle] if block ~ ~-0.5 ~ minecraft:air run tellraw @a[scores={PZGlBW.1Schl=3..},nbt={OnGround:true}] ["Gleisbettwandler.1:\n",{"text":"Es wurde kein Block auf den Konstruktionsblock gesetzt um das Gleisbett festzulegen!","color":"red","bold":true}]
