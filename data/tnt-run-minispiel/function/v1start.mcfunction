...
summon minecraft:area_effect_cloud ~ ~ ~ {Duration:400,custom_particle:{type:"minecraft:block",block_state:"minecraft:air"},Radius:0.0f,Tags:["EtiTNTRMS.1Start"]}
...
execute at @n[type=minecraft:text_display,tag=EtiTNTRMS.1Teleporter] run place template tnt-run-minispiel:v1teleporter ~-8 ~-3 ~-9 none

summon minecraft:marker ~ ~ ~-45 {Tags:["EtiTNTRMS.1Alle","EtiTNTRMS.1Spielfeld"]}
execute at @n[type=minecraft:marker,tag=EtiTNTRMS.1Spielfeld] run place template tnt-run-minispiel:v1gebiet ~ ~ ~ none
execute at @n[type=minecraft:marker,tag=EtiTNTRMS.1Spielfeld] run place template tnt-run-minispiel:v1gebiet ~ ~ ~ clockwise_90
execute at @n[type=minecraft:marker,tag=EtiTNTRMS.1Spielfeld] run place template tnt-run-minispiel:v1gebiet ~ ~ ~ 180
execute at @n[type=minecraft:marker,tag=EtiTNTRMS.1Spielfeld] run place template tnt-run-minispiel:v1gebiet ~ ~ ~ counterclockwise_90

execute at @n[type=minecraft:marker,tag=EtiTNTRMS.1Spielfeld] run summon minecraft:marker ~ ~3 ~ {Tags:["EtiTNTRMS.1Alle","EtiTNTRMS.1Ebene"]}
execute at @n[type=minecraft:marker,tag=EtiTNTRMS.1Spielfeld] run summon minecraft:marker ~ ~13 ~ {Tags:["EtiTNTRMS.1Alle","EtiTNTRMS.1Ebene"]}

execute as @n[type=minecraft:marker,tag=EtiTNTRMS.1Spielfeld] store result score @s PZTNTRMS.1Wert run data get entity @s Pos[1]
execute as @n[type=minecraft:marker,tag=EtiTNTRMS.1Spielfeld] store result entity @s data."EigTNTRMS.1Argumente".Y int 1 run scoreboard players add @s PZTNTRMS.1Wert 20

execute at @n[type=minecraft:marker,tag=EtiTNTRMS.1Spielfeld] run summon minecraft:interaction ~ ~21.5 ~ {width:48,height:3,Tags:["EtiTNTRMS.1Alle","EtiTNTRMS.1Wartesaal"]}

execute at @n[type=minecraft:marker,tag=EtiTNTRMS.1Spielfeld] positioned ~-30 ~-1 ~-30 as @a[dx=60,dy=40,dz=60] at @s run function tnt-run-minispiel:v1verlassen
