...
execute rotated ~ 0 run fill ^4 ^ ^1 ^-4 ^11 ^-1 minecraft:light_gray_stained_glass replace #gleisbruecken-_und_-tunnelbaumaschine:v3berg_und_meeres_bloecke
...
execute rotated ~ 0 run setblock ^-3 ^-2 ^-1 minecraft:air replace
execute at @e[distance=..5,type=minecraft:armor_stand,tag=EtiGBuTB.3Gleisbett1,sort=nearest,limit=4] rotated ~ 0 rotated ~ 0 run setblock ~ ~ ~ minecraft:andesite replace
...
scoreboard players add @s PZGBuTB.3Schl 1
execute if entity @s[scores={PZGBuTB.3Schl=2..}] at @e[distance=..5,type=minecraft:armor_stand,tag=EtiGBuTB.3Schwelle,sort=nearest,limit=3] rotated ~ 0 run setblock ~ ~1 ~ minecraft:spruce_slab replace
scoreboard players set @s[scores={PZGBuTB.3Schl=2..}] PZGBuTB.3Schl 0

# In einem bestimmten Abstand wird ein Oberleitungsmast durch die geladene Funktion platziert, dazu wird ein Zähler hochgezählt.
scoreboard players add @s PZGBuTB.3Pfeil 1
execute if entity @s[scores={PZGBuTB.3Pfeil=28..}] run function gleisbruecken-_und_-tunnelbaumaschine:v3bruecke_verlegen_2
scoreboard players set @s[scores={PZGBuTB.3Pfeil=28..}] PZGBuTB.3Pfeil 0

# Anschließend wird der Gleisbaumaschine exakt geradeaus von seiner Blickrichtung nach vorne teleportiert.
