...
execute at @e[distance=..5,type=minecraft:armor_stand,tag=EtiGleisB.3Gleisbett1,sort=nearest,limit=4] rotated ~ 0 rotated ~ 0 run setblock ~ ~ ~ minecraft:andesite replace
execute at @e[distance=..5,type=minecraft:armor_stand,tag=EtiGleisB.3Gleisspur,sort=nearest,limit=4] rotated ~ 0 rotated ~ 0 run setblock ~ ~ ~ minecraft:smooth_stone replace
...
scoreboard players add @s PZGleisB.3Schl 1
execute if entity @s[scores={PZGleisB.3Schl=2..}] at @e[distance=..5,type=minecraft:armor_stand,tag=EtiGleisB.3Schwelle,sort=nearest,limit=3] rotated ~ 0 run setblock ~ ~1 ~ minecraft:spruce_slab replace
scoreboard players set @s[scores={PZGleisB.3Schl=2..}] PZGleisB.3Schl 0

# Für die Oberleitung der Eisenbahn wird in einem bestimmten Abstand ein Oberleitungsmast aufgestellt
scoreboard players add @s PZGleisB.3Pfeil 1
execute if entity @s[scores={PZGleisB.3Pfeil=28..}] run function gleisbaumaschine:v3gleis_verlegen_2
scoreboard players set @s[scores={PZGleisB.3Pfeil=28..}] PZGleisB.3Pfeil 0

# Je nach Blickrichtung, wird der Gleisbaumaschine entsprechend bewegt
