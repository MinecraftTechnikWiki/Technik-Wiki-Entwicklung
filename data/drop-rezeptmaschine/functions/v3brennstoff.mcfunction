...
scoreboard players set @e[distance=0,type=minecraft:item,tag=EtiDrRezM.3Drop,scores={PZDrRezM.3Drop=0},sort=nearest,limit=1] PZDrRezM.3Fort 1200
...
#scoreboard players operation @e[distance=0,type=minecraft:item,tag=EtiDrRezM.3Drop,scores={PZDrRezM.3Drop=0},sort=nearest,limit=1] PZDrRezM.3Fort = VarDrRezM.3Brauen PZDrRezM.3Fort

#execute if entity @s[scores={PZDrRezM.3Drop=-3}] at @s run summon minecraft:item ~ ~ ~ {PickupDelay:2s,Item:{id:"minecraft:bucket",Count:1b},Tags:["EtiDrRezM.3Alle"],Motion:[0.0d,0.3d,0.0d]}
