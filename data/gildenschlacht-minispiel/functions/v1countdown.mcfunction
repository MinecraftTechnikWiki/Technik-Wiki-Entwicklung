...
#execute if score #VarGildSMS.1TickCountdown PZGildSMS.1Spiel matches 0 at @e[type=minecraft:armor_stand,tag=EtiGildSMS.1Spielfeld] as @a unless entity @s[team=!TMGildSMS.1Rot,team=!TMGildSMS.1Blau] positioned ~-25 ~ ~-25 run tellraw @s[dx=50,dy=50,dz=50] ["Gildenschlacht-Minispiel.1: ",{"text":"Spielstart in ","color":"gray","bold":true},{"score":{"name":"#VarGildSMS.1Countdown","objective":"PZGildSMS.1Spiel"} }]
...
#execute at @e[type=minecraft:armor_stand,tag=EtiGildSMS.1Spielfeld] run tellraw @a[distance=..50]
