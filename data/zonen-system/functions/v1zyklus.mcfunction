...
execute if entity @s[tag=!EtiZone.1SpielerNummer] run function zonen-system:v1schritt1

# Hat der Spieler eine Truhe geworfen, wird die Funktion dafür geladen
execute if entity @s[scores={PZZone.1Truhe=1..}] run function zonen-system:v1schritt2

# Sobald eine Option angeklickt hat, wird die Funktion dafür gestartet
execute if entity @s[scores={PZZone.1Eing=1..}] run function zonen-system:v1schritt3
...
execute at @s at @e[distance=..25,sort=nearest,limit=1,type=minecraft:armor_stand,name=Zone,tag=!EtiZone.1Zone] align xz positioned ~0.5 ~ ~0.5 run function zonen-system:v1schritt4
