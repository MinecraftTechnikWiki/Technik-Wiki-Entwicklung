...
execute as @e[distance=..2,tag=EtiSchMS.1Dame,tag=EtiSchMS.1Farbe] unless score @s PZSchMS.1Farbe matches 0.. run data merge entity @s {CustomNameVisible:true,CustomName:'{"text":"♕","bold":true}',HandItems:[{},{id:"minecraft:diamond_sword",Count:1b}],ArmorItems:[{},{id:"minecraft:leather_leggings",Count:1b,tag:{Unbreakable:true,display:{color:16383998}}},{id:"minecraft:leather_chestplate",Count:1b,tag:{Unbreakable:true,display:{color:16383998}}},{id:"minecraft:diamond_helmet",Count:1b,tag:{Unbreakable:true}}]}

execute if score VarSchMS.1Modelle PZSchMS.1Brett matches 1 as @e[distance=..2,tag=EtiSchMS.1Farbe,tag=EtiSchMS.1Dame,sort=nearest,limit=2] run data merge entity @s {HandItems:[{},{}],ArmorItems:[{},{},{},{id:"minecraft:feather",Count:1b,tag:{Unbreakable:true,display:{color:16383998}}}],ActiveEffects:[{Id:14b,Amplifier:127b,ShowParticles:false,Duration:2147483647}]}
execute if score VarSchMS.1Modelle PZSchMS.1Brett matches 1 run item replace entity @e[distance=..2,tag=EtiSchMS.1Dame,tag=EtiSchMS.1Anklickbar,sort=nearest,limit=1] armor.head with minecraft:leather_chestplate{Unbreakable:true,Damage:5,display:{color:16383998}}
