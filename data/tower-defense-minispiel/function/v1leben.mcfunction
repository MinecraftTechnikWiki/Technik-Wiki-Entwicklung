...
execute if entity @s[tag=EtiTowDefMS.1Gegner] store result score VarTowDefMS.1Max PZTowDefMS.1Leben run attribute @s minecraft:max_health get
...
execute if entity @s[tag=EtiTowDefMS.1Schuetze] positioned ~ ~2 ~ run data modify entity @n[distance=..5,type=minecraft:text_display,tag=EtiTowDefMS.1Aufladen] text set value ["",{text:"S: ",color:"red"},{storage:"tower-defense-minispiel:v1daten",nbt:'"EigTowDefMS.1Leiste"[]',separator:"",interpret:true}]
...
#data modify entity @s CustomName set from entity @n[distance=..5,type=minecraft:text_display,tag=EtiTowDefMS.1Leiste] text

#kill @n[distance=..5,type=minecraft:text_display,tag=EtiTowDefMS.1Leiste]
