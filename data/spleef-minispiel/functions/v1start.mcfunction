...
summon minecraft:area_effect_cloud ~ ~ ~ {Duration:400,Tags:["EtiSpleeMS.1Start"]}
...
execute at @e[type=minecraft:text_display,tag=EtiSpleeMS.1Teleporter,sort=nearest,limit=1] run place template spleef-minispiel:v1teleporter ~-8 ~-3 ~-9 none

summon minecraft:marker ~ ~ ~-50 {Tags:["EtiSpleeMS.1Alle","EtiSpleeMS.1Spielfeld"]}
execute at @e[type=minecraft:marker,tag=EtiSpleeMS.1Spielfeld,sort=nearest,limit=1] run place template spleef-minispiel:v1gebiet ~ ~ ~ none
execute at @e[type=minecraft:marker,tag=EtiSpleeMS.1Spielfeld,sort=nearest,limit=1] run place template spleef-minispiel:v1gebiet ~ ~ ~ clockwise_90
execute at @e[type=minecraft:marker,tag=EtiSpleeMS.1Spielfeld,sort=nearest,limit=1] run place template spleef-minispiel:v1gebiet ~ ~ ~ 180
execute at @e[type=minecraft:marker,tag=EtiSpleeMS.1Spielfeld,sort=nearest,limit=1] run place template spleef-minispiel:v1gebiet ~ ~ ~ counterclockwise_90

execute at @e[type=minecraft:marker,tag=EtiSpleeMS.1Spielfeld,sort=nearest,limit=1] run summon minecraft:marker ~ ~5 ~ {Tags:["EtiSpleeMS.1Alle","EtiSpleeMS.1Ebene"]}

execute as @e[type=minecraft:marker,tag=EtiSpleeMS.1Spielfeld,sort=nearest,limit=1] store result score @s PZSpleeMS.1Wert run data get entity @s Pos[1]
execute as @e[type=minecraft:marker,tag=EtiSpleeMS.1Spielfeld,sort=nearest,limit=1] store result entity @s data."EigSpleeMS.1Argumente".Y int 1 run scoreboard players add @s PZSpleeMS.1Wert 10

execute at @e[type=minecraft:marker,tag=EtiSpleeMS.1Spielfeld,sort=nearest,limit=1] run summon minecraft:interaction ~ ~18.5 ~ {width:52,height:3,Tags:["EtiSpleeMS.1Alle","EtiSpleeMS.1Wartesaal"]}

execute at @e[type=minecraft:marker,tag=EtiSpleeMS.1Spielfeld,sort=nearest,limit=1] positioned ~-40 ~-1 ~-40 as @a[dx=80,dy=40,dz=80] at @s run function spleef-minispiel:v1verlassen
