...
execute if score VarRSchMS.1Hoehe PZRSchMS.1Brett < VarRSchMS.1HoeheMax PZRSchMS.1Brett run summon minecraft:area_effect_cloud ~ ~8 ~ {NoGravity:true,Duration:-1,Age:-2147483648,WaitTime:-2147483648,Particle:"minecraft:block minecraft:air",Tags:["EtiRSchMS.1Alle","EtiRSchMS.1Feld"]}
execute if score VarRSchMS.1Hoehe PZRSchMS.1Brett < VarRSchMS.1HoeheMax PZRSchMS.1Brett positioned ~ ~8 ~ as @e[distance=..3,type=minecraft:area_effect_cloud,tag=EtiRSchMS.1Feld,sort=nearest,limit=1] run function raumschach-minispiel:v1erstellen_hoehe
