...
execute positioned ^-3 ^ ^ if block ^1 ^-1 ^ minecraft:light_gray_concrete if score @e[distance=..1,type=minecraft:armor_stand,tag=EtiMADNMS.1Ziel,sort=nearest,limit=1] PZMADNMS.1Team = VarMADNMS.1Farbe PZMADNMS.1Team positioned ^3 ^ ^3 if score @e[distance=..1,type=minecraft:armor_stand,tag=EtiMADNMS.1Startfeld,tag=EtiMADNMS.1Team,sort=nearest,limit=1] PZMADNMS.1Team = VarMADNMS.1Farbe PZMADNMS.1Team at @s run teleport @s ~ ~ ~ ~90 ~
...
execute if entity @s[tag=!EtiMADNMS.1SchrittAbrechen,scores={PZMADNMS.1Drop=-1}] if score @e[distance=..1,type=minecraft:armor_stand,tag=EtiMADNMS.1Figur,sort=nearest,limit=1] PZMADNMS.1Team = VarMADNMS.1Farbe PZMADNMS.1Team run tag @s add EtiMADNMS.1SchrittAbrechen
